...
from compgraph import DcnCompGraph

parser = argparse.ArgumentParser(description="Deep k-means algorithm.")
parser.add_argument("-d", "--dataset", type=str.upper,
                    help="Dataset on which DCN will be run (one of USPS, MNIST, 20NEWS, RCV1)", required=True)
...
pretrain = args.pretrain # Specify if DCN's autoencoder should be pretrained
...
for run in range(n_runs):
    ...
    cg = DcnCompGraph([specs.dimensions, specs.activations, specs.names], specs.n_clusters, batch_size, specs.n_samples,
                      lambda_)
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        for epoch in range(n_finetuning_epochs):
            ...
            print_val = 1
            if epoch % print_val == 0 or epoch == n_finetuning_epochs - 1:
                print("loss:", loss_)
                print("ae loss:", ae_loss_)
                print("kmeans loss:", kmeans_loss_)

                if validation:
                    validation_cluster_assign = np.asarray([cluster_assign_[i] for i in specs.validation_indices])
                    test_cluster_assign = np.asarray([cluster_assign_[i] for i in specs.test_indices])
                    ...
                    # Evaluate the clustering performance using the ground-truth labels
                    acc = cluster_acc(target, cluster_assign_)
                    print("ACC", acc)
                    ari = adjusted_rand_score(target, cluster_assign_)
                    print("ARI", ari)
                    nmi = normalized_mutual_info_score(target, cluster_assign_)
                    print("NMI", nmi)
